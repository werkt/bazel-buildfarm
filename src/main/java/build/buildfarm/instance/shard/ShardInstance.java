...
public class ShardInstance extends AbstractServerInstance {
    ...
    return allAsList(
        Iterables.transform(
            ...
            digest ->
                catching(
                    transform(
                        getBlobFuture(Compressor.Value.IDENTITY, digest, RequestMetadata.getDefaultInstance()),
                        blob -> {
        ...
        new WorkersCallback(rand) {
          ...
          public void onQueue(Deque<String> workers) {
            ctx.run(
                () ->
                    fetchBlobFromWorker(
                        compressor, blobDigest, workers, offset, count, chunkObserver, requestMetadata));
          }
  ...
  private ListenableFuture<Long> writeBlobFuture(
    ...
    Write write = getBlobWrite(Compressor.Value.IDENTITY, digest, UUID.randomUUID(), requestMetadata);
    addCallback(
        write.getFuture(),
