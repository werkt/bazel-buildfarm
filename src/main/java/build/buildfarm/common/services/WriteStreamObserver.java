...
public class WriteStreamObserver implements StreamObserver<WriteRequest> {
  ...
  @GuardedBy("this")
  void onUncommittedNext(WriteRequest request) throws EntryLimitException, InvalidResourceNameException {
    if (initialized) {
    ...
    switch (detectResourceOperation(resourceName)) {
      case UPLOAD_BLOB_REQUEST:
        ...
        return ByteStreamService.getUploadBlobWrite(
            instance, parseUploadBlobCompressor(resourceName), uploadBlobDigest, parseUploadBlobUUID(resourceName));
