...
public class ByteStreamService extends ByteStreamImplBase {
  ...
  static Write getUploadBlobWrite(Instance instance, Compressor.Value compressor, Digest digest, UUID uuid)
      throws EntryLimitException {
    if (digest.getSizeBytes() == 0) {
      return new CompleteWrite(0);
    }
    return instance.getBlobWrite(compressor, digest, uuid, TracingMetadataUtils.fromCurrentContext());
  }
    ...
    switch (detectResourceOperation(resourceName)) {
      ...
      case UPLOAD_BLOB_REQUEST:
        return getUploadBlobWrite(
            instance, parseUploadBlobCompressor(resourceName), parseUploadBlobDigest(resourceName), parseUploadBlobUUID(resourceName));
      case STREAM_OPERATION_REQUEST:
