...
class Cat {
    ...
    protected void run(Instance instance, Digest digest) throws Exception {
      try (InputStream in =
          instance.newBlobInput(
              Compressor.Value.IDENTITY, digest, 0, 60, TimeUnit.SECONDS, RequestMetadata.getDefaultInstance())) {
  ...
  abstract static class BlobCommand extends DigestsCommand {
    @Override
    protected void run(Instance instance, Digest digest) throws Exception {
      run(instance, getBlob(instance, Compressor.Value.IDENTITY, digest, RequestMetadata.getDefaultInstance()));
    }
