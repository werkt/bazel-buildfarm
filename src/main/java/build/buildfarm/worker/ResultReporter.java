...
class ResultReporter implements Runnable {
  ...
  ResultReporter(WorkerContext workerContext, ExecutionContext executionContext, ReportResultStage owner) {
  ...
  private long reportPolled(Stopwatch stopwatch)
  ...
  private void proceedToOutput(Operation completedExecution) throws InterruptedException {
    ...
    if (claimed) {
      ...
      String executionName = executionContext.queueEntry.getExecuteEntry().getOperationName();
      log.log(Level.FINER, "ResultReporter: Execution " + executionName + " Failed to claim output");
