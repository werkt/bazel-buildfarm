...
class InvocationsCollector extends LinkedBlockingQueue<RequestMetadata> implements Runnable {
  ...
  public void run() {
    for (;;) {
      ...
      try {
        ...
        log.log(WARNING, "error handling invocation " + meta.getCorrelatedInvocationsId() + " => " + meta.getToolInvocationId(), e);
      }
    }
  }

  private void addNewToolInvocationId(UUID toolInvocationId, String correlatedInvocations, ToolDetails toolDetails) throws IOException {
    ...
    if (len > UUID_STRING_LENGTH) {
      try {
        ...
        correlatedInvocationsId = UUID.fromString(correlatedInvocations.substring(len - UUID_STRING_LENGTH));
      }
    } else {
  ...
  private void iterate(RequestMetadata meta) throws IOException {
    ...
    if (seen.add(toolInvocationId)) {
      addNewToolInvocationId(toolInvocationId, meta.getCorrelatedInvocationsId(), meta.getToolDetails());
    }

    instance.addRequest(
        meta.getActionId(),
        toolInvocationId,
        meta.getActionMnemonic(),
        meta.getTargetId());
  }
}
